...
void UpdatePlayer()
{
	...
	if (Keyboard_IsPress(DIK_RIGHT) && Keyboard_IsPress(DIK_UP))    //右上
	{
		angle = 315;
		g_player.direction = RIGHT;
	}
	else if (Keyboard_IsPress(DIK_LEFT) && Keyboard_IsPress(DIK_UP))    //左上
	{
		angle = 225;
		g_player.direction = LEFT;
	}
	else if (Keyboard_IsPress(DIK_LEFT) && Keyboard_IsPress(DIK_DOWN))    //左下
	{
		angle = 135;
		g_player.direction = LEFT;

	}
	else if (Keyboard_IsPress(DIK_RIGHT) && Keyboard_IsPress(DIK_DOWN))    //右下
	{
		angle = 45;
		g_player.direction = RIGHT;
	...
	g_player.position.y = max(g_player.position.y, g_player.height*0.5);
	g_player.position.y = min(g_player.position.y, SCREEN_HEIGHT - g_player.height*0.5);
	/*	関数化可能	*/
